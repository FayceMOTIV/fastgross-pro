  ...
  return (
    <div className="fixed inset-0 z-[100]">
      ...
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg">
        <div className="bg-gray-50 border border-gray-300 rounded-2xl shadow-2xl overflow-hidden">
          {/* Header */}
          ...
          <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
            <p className="text-xs text-gray-400 text-center">
              Appuyez sur{' '}
              <kbd className="px-1.5 py-0.5 bg-gray-100 rounded border border-gray-300 text-gray-500">
                ESC
              </kbd>{' '}
              pour fermer
            </p>
  ...
  return (
    <div className="flex items-center justify-between py-2">
      ...
      <div className="flex items-center gap-1">
        {keys.map((key, i) => (
          <span key={i}>
            <kbd className="px-2 py-1 text-xs font-medium text-gray-600 bg-gray-100 rounded border border-gray-300">
              {key}
            </kbd>
            {i < keys.length - 1 && <span className="text-gray-300 mx-0.5">+</span>}
          </span>
        ))}
      </div>
    </div>
