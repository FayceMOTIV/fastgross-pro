...
export const isJunkEmail = (email) => {
  ...
  return JUNK_EMAIL_PATTERNS.some((pattern) => emailLower.includes(pattern.toLowerCase()))
}
...
export const getEmailPriority = (email) => {
  ...
  const genericPrefixes = Object.keys(EMAIL_PRIORITY_PREFIXES).filter(
    (k) => k !== 'personal' && k !== 'default'
  )
...
export const extractPhoneFromHtml = (html) => {
  ...
  if (matches.length > 0) {
    ...
    return matches[0].replace(/[\s.-]/g, '').replace(/^(?:\+33|0033)/, '0')
  }
...
export const extractEmailsFromSite = async (url) => {
  ...
  try {
    ...
    const pagesToCheck = EMAIL_EXTRACTION_PAGES.map((page) => (page ? `${baseUrl}${page}` : url))
