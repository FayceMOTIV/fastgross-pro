...
export function useEmailAccounts() {
  ...
  const addSmtpAccount = useCallback(
    async (config) => {
  ...
  const addGmailAccount = useCallback(
    async (oauthData) => {
      if (!currentOrg?.id) throw new Error('Organisation non selectionnee')
      ...
      const newAccount = {
        ...
        credentials,
        status: ACCOUNT_STATUS.WARMING_UP,
        dailyLimit: oauthData.isWorkspace
  ...
  const addOutlookAccount = useCallback(
    async (oauthData) => {
      ...
      const newAccount = {
        ...
        credentials,
        status: ACCOUNT_STATUS.WARMING_UP,
        dailyLimit: oauthData.isBusiness
  ...
  const toggleAccountStatus = useCallback(
    async (accountId, currentStatus) => {
      const newStatus =
        currentStatus === ACCOUNT_STATUS.PAUSED ? ACCOUNT_STATUS.ACTIVE : ACCOUNT_STATUS.PAUSED
  ...
  const stats = {
    ...
    activeCount: accounts.filter(
      (a) => a.status === ACCOUNT_STATUS.ACTIVE || a.status === ACCOUNT_STATUS.WARMING_UP
    ).length,
