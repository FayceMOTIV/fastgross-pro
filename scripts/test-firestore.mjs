...
// Initialize Firebase Admin (uses ADC from gcloud or service account)
let app
try {
...
async function testFirestore() {
  ...
  const mockProspects = [
    {
      ...
      hooks: [
        "J'ai remarque votre Bistrot des Amis recemment ouvert - votre concept est unique.",
        "Vos 2.1K followers montrent un potentiel enorme - du contenu video pourrait tripler ce chiffre.",
        "Et si on automatisait votre creation de contenu pour garder vos clients engages ?"
      ],
      status: 'pending'
    },
    {
      ...
      hooks: [
        "Claire, votre contenu culinaire est superbe - la video courte pourrait vous faire exploser sur Reels.",
        "Avec vos 11.5K followers, des videos pros pourraient attirer des marques.",
        "J'ai vu votre post sur les partenariats - et si on creait du contenu qui vous demarque ?"
      ],
      status: 'pending'
    },
    {
      ...
      hooks: [
        "Votre boulangerie a un potentiel Instagram incroyable - vos creations meritent d'etre filmees.",
        "3.8K followers pour une boulangerie artisanale comme la votre, c'est bien - mais on peut faire x5.",
        "Des videos de fabrication de vos pains et patisseries feraient un carton sur Instagram."
      ],
      status: 'pending'
    }
  ]
  ...
  try {
    // Step 1: Save prospects to Firestore
    ...
    // Step 3: Query hot prospects (like dashboard)
    ...
    results.autopilotWorkflow = {
      ...
      configSaved: true,
      configVerified: configDoc.exists,
      cleanedUp: true
    }

  } catch (error) {
    console.log(`  AutoPilot workflow: FAILED - ${error.message}`)
    console.log(`  Stack: ${error.stack}`)
    results.autopilotWorkflow = { success: false, error: error.message }
  }

  // ---- Test 4: Check existing AutoPilot configs ----
  console.log()
  console.log('-'.repeat(60))
  console.log('TEST 4: Existing AutoPilot Data Check')
  console.log('-'.repeat(60))

  try {
    // Check if any real orgs have autopilot configured
    ...
    console.log(`  Total orgs with autopilot config: ${autopilotOrgs}/${orgs.size}`)
