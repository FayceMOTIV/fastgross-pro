...
// ═══════════════════════════════════════════════════════════════════════════
// CATALOGUE DISTRAM - Version Realiste avec structure SAGE
// ═══════════════════════════════════════════════════════════════════════════
...
const CATALOGUE_DISTRAM: ProduitCatalogue[] = [
  // VIANDES - Kebab
...
interface ProduitRecommande {
  ...
  nom: string;
  categorie: string;
  quantite: number;      // TOUJOURS 1
  unite: string;
  prixUnitaire: number;
  totalHT: number;
  raison: string;
  obligatoire: boolean;
  contenance: string;    // Info pour le commercial
}
...
function mapToDistramProducts(plats: PlatDetecte[], typeRestaurant: string): {
  produitsRecommandes: ProduitRecommande[];
  emballagesRecommandes: ProduitRecommande[];
} {
  const produitsRecommandes: ProduitRecommande[] = [];
  const emballagesRecommandes: ProduitRecommande[] = [];
  const produitsAjoutes = new Set<string>();
  const emballagesAjoutes = new Set<string>();

  // Collecter tous les ingredients
  const tousIngredients: string[] = [];
  plats.forEach(plat => {
    if (plat.ingredients) {
  ...
  const ajouterProduit = (ref: string, raison: string, obligatoire: boolean = true) => {
    if (produitsAjoutes.has(ref)) return;
  ...
  const ajouterEmballage = (ref: string, raison: string, obligatoire: boolean = true) => {
    if (emballagesAjoutes.has(ref)) return;
    const produit = CATALOGUE_DISTRAM.find(p => p.ref === ref);
    if (!produit) return;
  ...
  const hasIngredient = (keywords: string[]) =>
    ingredientsLower.some(ing => keywords.some(kw => ing.includes(kw)));
...
export const scanMenu = onRequest(
  {
    cors: true,
    memory: '512MiB',
    timeoutSeconds: 120,
...
RETOURNE UNIQUEMENT ce JSON (pas de texte avant/apres) :
{
  ...
  "notes": ["Ce restaurant semble specialise en burgers premium", "Plusieurs fromages speciaux detectes"]
      ...
      const { produitsRecommandes, emballagesRecommandes } = mapToDistramProducts(
        analysisData.plats || [],
        analysisData.restaurant?.type || 'mixte'
      );
